// ═══════════════════════════════════════════════════════════════
// TIPS Dashboard — dataLoader.js  (fixed)
// Fetch and parse all 9 pipeline output files
// ═══════════════════════════════════════════════════════════════
...
// ── Parse JSONL for relevance_scores (one JSON object per line) ─
function parseRelevanceJSONL(text) {
    ...
    // Fallback: might be a regular JSON array
    if (!scores.length) {
        try {
...
// ── Audio aggregation (avoid rendering 600 KB raw) ────────────
...
function _store(key, data) {
    switch (key) {
        case 'timeline':
            ...
            break;
        case 'qa_pairs':
            // Structure: { dataset_id, total_pairs, qa_pairs: [...] }
        ...
        case 'relevance_scores':
            // JSONL: each line is one { qa_id, relevance_score, matched_keywords, ... }
            sessionData.relevance_scores = Array.isArray(data) ? data : [];
            break;
        case 'behavior_metrics':
            // Structure: { dataset_id, segment_count, segments: [...] }
            sessionData.behavior_metrics = data;
            break;
        case 'speaking_segments':
            // Structure: { dataset_id, segments: [...] }
        ...
        case 'video_raw':
            ...
            break;
        case 'interviewer':
            // Structure: { dataset_id, source_file, transcription: { language, segments: [...] } }
...
// All segments in speaking_segments.json are type="speaking" — sum all
